...
### Simple multiple regress with tf.contrib.learn
import tensorflow as tf

features = [tf.contrib.layers.real_valued_column('nasdaq_x', dimension=1), tf.contrib.layers.real_valued_column('oil_x', dimension=1)]

estimator = tf.contrib.learn.LinearRegressor(feature_columns=features)

input_fn = tf.contrib.learn.io.numpy_input_fn(x={"nasdaq_x": nasdaqData, "oil_x": oilData}, y=xomData, batch_size=len(nasdaqData), num_epochs=100000)
